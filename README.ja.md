...
1. プロキシを起動
   ...
   ```
2. クライアントを `http://localhost:8765/mcp` に向ける
...
- STDIO は NDJSON で応答（Codex 互換）

## アーキテクチャ

プロセス構成は [Docs/architecture.md](Docs/architecture.md) を参照してください。
...
| `--transport mode` | 転送モード: `http`, `stdio`, `both` |
...
- STDIO は単一セッション（1プロセス1クライアント）です。
